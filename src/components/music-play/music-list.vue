<template>
	<div class="list-div">
		<div class="header">
			<div class="left">{{`共${playlist.length}首`}}</div>
			<div class="right">
				<div class="one">
					<i class="iconfont icon-iconset0196" title="收藏全部"></i>
					<span>收藏全部</span>
				</div>
				<div>
		...
		<div class="item-music" v-for="(item, index) in playlist" :key="index + 'play'" @dblclick="playThis(item, index)">
...
export default {
	...
	computed: {
	  ...mapGetters([ 'currentMusic', 'playlist', 'playing' ]),
	},
